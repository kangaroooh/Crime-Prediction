...
#%%
nlp = StanfordCoreNLP(
    "/home/r/Codes/txt-mng/stanford-corenlp-full-2018-10-05",
    memory='6g',
    timeout=3000)
props = {
...
for comment in tqdm(titles):
    results_title.append(
        json.loads(nlp.annotate(
            comment, properties=props))['sentences'][0]['sentiment'])


#%%
def get_sentiment(comment):
    return json.loads(nlp.annotate(
        comment, properties=props))['sentences'][0]['sentiment']
...
for comment in tqdm(contents):
    results_content.append(
        json.loads(nlp.annotate(
            comment[:100], properties=props))['sentences'][0]['sentiment'])
...
json.loads(nlp.annotate(contents.iloc[2],
                        properties=props))['sentences'][0]['sentiment']
